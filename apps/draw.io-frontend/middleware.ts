...
export const HTTP_URL = process.env.NEXT_PUBLIC_HTTP_URL || 'http://localhost:4000';
...
async function verifyToken(token: string) {
    console.log('Verifying token:', token); // Added logging
    console.log('Using secret:', JWT_SECRET);
  try {
    const secret = new TextEncoder().encode(JWT_SECRET);
    const { payload } = await jwtVerify(token, secret, {
      algorithms: ['HS256'] // Explicitly specify the algorithm used by jsonwebtoken
    });
    ...
    console.error('Error details:', {
      ...
      secretValue: JWT_SECRET === 'your_jwt_secret' ? 'using fallback secret' : 'using env secret'
    });
    return null;
  }
}
